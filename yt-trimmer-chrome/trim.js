/* Main function to be injected. */
function trim() {
  let extUi = false;
  let dict  = {};
  let container, searchBar, widgetElem, statusElem,
      formElem, inputStart, inputEnd, boxElem;

  /* Update extUi or dict on message from content script. */
  ...
  function initWidget() {
    ...
    container  = document.getElementById('yt-masthead-content');
    searchBar  = document.getElementById('masthead-search');

    /* Assign separate css based on YouTube layout. */
    if (!container) {
      layout    = 'new-layout'
      container = document.querySelector('ytd-searchbox.style-scope');
      searchBar = document.getElementById('search-form');
    }

    widgetElem = document.createElement('span');
    widgetElem.id = 'trim-widget';
    widgetElem.classList = layout;
  ...
  function initWidgetListeners() {
    ...
    function hideStatus(event) {
      document.getElementById('trim-box').classList.remove('show-status');
    }
    ...
    function updateEntry(event) {
       ...
       */
      if (player.getVideoData['isLive']) showNoti(3);
      ...
      function showNoti(code) {
        ...
        setTimeout(() => { boxElem.classList.remove('show-noti') }, 2500);
      }
    }
  ...
  function widgetStep() {
     ...
     * video is playing.
     */
    if (!extUi && widgetElem) widgetElem.style.display = 'none';
    else if (!vidId || !vidLength) hideWidget();
    else {
      let ctWidth = container.offsetWidth;
      let sbWidth = searchBar.offsetWidth;
      let wgWidth = widgetElem.offsetWidth;
      if (container && searchBar && ctWidth - sbWidth < wgWidth)
        hideWidget();
      else showWidget();
